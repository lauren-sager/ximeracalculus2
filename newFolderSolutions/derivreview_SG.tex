...
\title{Derivative Review Activity Solution Guide }
...
\begin{exercise}
    ...
    \textcolor{blue}{This is subjective and unique to your group!  I would suggest that facts about limits at infinity might be helpful for later in Calculus II. MAC tutors and LAs also suggested that trig function review, the Pythagorean trig identites, and log and exponent rules will be helpful!} 
    %Examples:\\
    %l'H\^opital's Rule: $\lim_{x\rightarrow a}\frac{f(x)}{g(x)}=\lim_{x\rightarrow a}\frac{f'(x)}{g'(x)}$ \textbf{ if} both $\lim_{x\rightarrow a}f(x)=0$ and $\lim_{x\rightarrow a}g(x)=0$ \textbf{or} $\lim_{x\rightarrow a}f(x)=\pm \infty$ and $\lim_{x\rightarrow a}g(x)=\pm \infty$ (i.e. $\frac{f(x)}{g(x)}$ gives a $\frac00$ or $\frac{\infty}{\infty}$ form).\\
    %Function dominance (a.k.a the fishy diagram)}
\end{exercise}
