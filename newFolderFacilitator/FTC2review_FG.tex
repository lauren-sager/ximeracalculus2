...
\title{Fundamental Theorem of Calculus Review 2 Activity Facilitator Guide}
...
Students will be able to:
    \begin{itemize}
        \item Compute the value of a simple linear integral using geometric methods.
        \item Compute the value of a simple linear integral using the power rule for integration.
        \item Compute the value of a simple lnear integral using the limit of a Riemann sum approximation.
        \item Compare answers using the three methods and check for agreement.
    \end{itemize}
\textbf{Prerequisite Knowledge:} Students should should be aware of the notation for definite integrals, and you will need to recall the power rule for integration. The technical details surrounding the area approximation are given in the handout, but you will need to use algebraic properties of limits and summation notation to manipulate the approximation and show agreement between the integral formula and the limit of the area approximations.\\
\textbf{Helpful formulas and summation properties to have on hand:} Students should be encouraged to use their notes or textbook to find these formulas (as a study skills/fact seeking exercise), but here they are for your use: \\
\textcolor{orange}{$\displaystyle\sum_{j=1}^n(a_j+b_j)=\sum_{j=1}^n a_j+\sum_{j=1}^n b_j$}\\

\textcolor{orange}{$\displaystyle \sum_{j=1}^n ca_j=c\sum_{j=1}^n a_j$, $c$ is any constant}\\
\textcolor{orange}{$\displaystyle \sum_{j=1}^n c=cn$, $c$ is any constant and $n\geq 1$}\\
\textcolor{orange}{$\displaystyle \sum_{j=1}^n j=\frac{n(n+1)}{2}$}\\
\textcolor{orange}{$\displaystyle \sum_{j=1}^n j^2 =\frac{n(n+1)(2n+1)}{6}$}\\

\textcolor{orange}{Power rule for integration: $\displaystyle\int x^ndx=\frac{x^{n+1}}{n+1}+c$, when $n\neq -1$}\\
...
$\displaystyle\lim_{n\rightarrow \infty}\sum_{j=1}^{n}(0+j(\frac2n))\frac{2}{n}=\lim_{n\rightarrow \infty}\sum_{j=1}^n j(\frac{4}{n^2})=\lim_{n\rightarrow \infty}\frac{4}{n^2}\sum_{j=1}^n j$}
        ...
        \textcolor{orange}{Students tend to find these somewhat imprecise scaffold parts frustrating sometimes.  Remind students that as long as they are following the correct steps and get there eventually, it doesn't really matter where they do the steps.}
        ...
        \textcolor{orange}{Generally, students are allowed to argue about what is happening with polynomial/polynomial with powers as well, as long as they can correctly explain the rule to us.  If you're not sure if this is acceptable or not in your situation, ask your instructor about their policy.}
