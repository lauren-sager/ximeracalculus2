...
\title{$u$-Substitution Activity Facilitator Guide}
...
Students will be able to:
    \begin{itemize}
        \item Evaluate integrals with a chain rule structure using $u$-substitution.
        \item Select an appropriate $u(x)$ for $u$-substitution.
        \item Identify $u$ and $\frac{du}{dx}$ pairs that will and will not work for the $u$-substitution method.
    \end{itemize}

\textbf{Prerequisite Knowledge:} This activity assumes students are familiar with the fundamental theorem of calculus, the power rule for integration and the derivative rules.\\
This is intended to be used after an introduction to $u$-sub, but does not require students to go beyond perfect $u$-sub.
    ...
    \begin{enumerate}
        ...
        \item Next you are going to evaluate your integral {\bf{three times}} and verify the values are the same.
            \begin{enumerate}
                ...
                \textcolor{orange}{Note that there is a subtle difference between method 2 and 3. You may have learned (and your student may have learned in a previous course) a slightly different method for $u$-sub. We are trying to be completely honest with the mathematics here, and be transparent about the fact that $\frac{du}{dx}$ is not really a fraction, so we cannot truly ``cancel the $dx$.''  While it is tempting to do so (and some students will try to do it this way after learning from videos), we really want to set students up in the best way possible and these types of tricks can come back to bite you in future math classes.  Students will not be penalized for using the other method, but please encourage them to interact with these directions and try it this way.}
...
\begin{exercise}
    ...
    \textcolor{blue}{Here are two examples to get you started:}\\
        \textcolor{blue}{
        With $u = x^2$, $\frac{du}{dx} = 2x$.  The table gives examples for both of the previous answers, along with the suggested $u$ that does/does not make the example work.}
        
        \begin{center}\begin{tabular}{cc}
        WILL WORK & WON'T WORK\\\hline
        $\int\frac{2x}{1+x^2}dx$ & $\int\frac{x^2}{1+2x}dx$ \\
        $u=1+x^2$ & $u=1+2x$ will work, but it creates a more complicated version of $u$-sub. \end{tabular}\end{center}
    ...
    \textcolor{blue}{
        ...
        %(Here $u=\tan \theta +1$ is a better choice on the will-work example, and the second example simplifies miraculously using Pythagorean identities. 
        \textcolor{orange}{The point you are trying to refine is that $u$-substitution is not `just' picking a function and its derivative from the integrand, but identifying a function and derivative \textbf{which follow the chain rule pattern}.}
